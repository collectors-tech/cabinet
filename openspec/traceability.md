...
## SHOP_PROVIDERS Migration (commit `da7c354`)
...
| `INTEGRATION-010` | Amazon disabled mode explicit `409 PROVIDER_DISABLED` envelope | provider-disabled API envelope contract harness | `TestAmazonDisabledModeReturns409ContractEnvelope` (`tests/shop_providers_contract_test.go`) | implemented |
| `OPS-001` | AU webshop throttling/backoff conformance and degraded state observability | scanner retry/backoff + provider health/failure logs | `TestAUWebshopThrottlingConformanceOPS001` (`tests/shop_providers_contract_test.go`) | implemented |
