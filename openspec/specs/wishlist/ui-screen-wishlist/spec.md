...
#### Scenario: Keep view controls inline

- **GIVEN** wishlist route is loaded at desktop width
- **WHEN** the Wishlist table toolbar renders
- **THEN** the `View`, `Rows`, and `Cards` controls MUST share the same toolbar line without wrapping into a second row
