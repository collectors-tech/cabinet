# chats

## Purpose
Contains chat/copilot, AI assist, and chat screen behavior.

## Contained Sub-specs
- `ai-assist` -> `openspec/specs/chats/ai-assist/spec.md`
- `assistant-execution-surfaces` -> `openspec/specs/chats/assistant-execution-surfaces/spec.md`
- `assistant-inbox-handoff` -> `openspec/specs/chats/assistant-inbox-handoff/spec.md`
- `assistant-workspace` -> `openspec/specs/chats/assistant-workspace/spec.md`
- `chat-copilot` -> `openspec/specs/chats/chat-copilot/spec.md`
- `chats-workspace` -> `openspec/specs/chats/chats-workspace/spec.md`
- `ui-screen-chat-copilot` -> `openspec/specs/chats/ui-screen-chat-copilot/spec.md`

## Requirement ID Namespaces
- `AI-ASSIST-*`
- `ASSISTANT-EXECUTION-*`
- `ASSISTANT-INBOX-*`
- `ASSISTANT-WORKSPACE-*`
- `CHAT-COPILOT-*`
- `CHATS-WORKSPACE-*`
- `UI-SCREEN-CHAT-COPILOT-*`
