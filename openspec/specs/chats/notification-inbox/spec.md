...
The Notification Inbox MUST render a compact dark two-pane operating layout with filter counts, visible/unread/system/hidden stat counters, a table-style paginated message list, total count, selected notification detail, and icon-only page/detail actions with accessible names. Clearing visible notifications MUST archive/hide the currently visible records without dropping them from the durable Inbox state, and Show hidden MUST reveal those archived records again.
...
- **WHEN** the route renders
- **THEN** it MUST show filter buttons with inline counts, visible/unread/system/hidden stat counters, a search field, compact table-style paginated rows, a total message count below the table, and a selected notification detail pane
- **AND** page actions such as refresh, mark all visible as read, clear all, show hidden, row triage, and selected-detail handled/delete MUST be icon-only with accessible names/tooltips.

### Requirement UI-SCREEN-NOTIFICATION-INBOX-008: Notification-like UI events SHALL be preserved in Inbox history
Cabinet notification-like UI events, including toast messages, promise-based success/failure feedback, shared confirmation or warning dialogs, and inline status/banner messages from Dashboard/Home API failures, operational settings, Settings Account, Appearance, Display, and Profile preference save/load or validation feedback, storage maintenance, runtime setup import operations, notification-preference settings success and failure outcomes, taxonomy settings, Collections workspace, Wishlist/Tasks save, import, bulk, screenshot, image drop, and barcode feedback, generic Task table bulk status, priority, export, and delete feedback, Users admin bulk action/delete confirmation feedback, Auth entry sign-in/sign-up/forgot-password/passkey feedback, global query/server error feedback, Integrations provider health surfaces, Integrations provider configuration save/disconnect feedback, Integrations buyer-interest, seller operation, listing lifecycle, and landed-cost action feedback, and Media workspace load, add, metadata edit, download preview, assignment, and image-analysis workflow feedback, MUST be captured into the Notification Inbox history so immediate feedback is not the only record. Captured records MUST include a source label, event time, level/category metadata, title, and detail or lifecycle summary sufficient for later review from the Inbox route. Promise feedback MUST preserve configured source/category metadata on loading and settled records. Once Cabinet has an active profile, captured local notification history MUST be promoted into the server-backed Inbox store and deduplicated by local capture ID so the record survives local history loss or reload.
...
- **GIVEN** a user triggers a Dashboard/Home API failure, operational settings, storage maintenance, runtime setup import, notification-preference settings, taxonomy settings, Collections workspace, Integrations provider health, provider configuration, buyer-interest, seller operation, listing lifecycle, landed-cost action, or Media workspace load/add/edit/download/assignment/analysis action that renders inline or toast-backed success or failure status copy
