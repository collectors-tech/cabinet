...
- **AND** Browse item summaries with listing id or title values containing raw ASCII control bytes after trimming MUST be skipped instead of creating candidates with unsafe text provenance.
...
- **AND** seller usernames containing raw ASCII control bytes after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
