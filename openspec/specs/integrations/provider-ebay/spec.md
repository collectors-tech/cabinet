...
- **AND** configured Browse base URL overrides MUST be trimmed and accepted only when they are HTTP(S) URLs without embedded userinfo, query strings, fragments, embedded raw ASCII or Unicode whitespace, raw Unicode control/format characters, percent-encoded ASCII spaces, percent-encoded Unicode whitespace or format/control characters, or raw or percent-encoded ASCII control bytes; blank, relative, non-web, userinfo-bearing, query-bearing, fragment-bearing, encoded-space, encoded-unsafe-Unicode, raw-unsafe-Unicode, raw-whitespace, or control-character override values MUST fall back to the default production eBay API base URL before authenticated requests are built.
