...
- **AND** Browse item summaries with non-HTTP(S) item URLs, oversized item URLs, URLs containing embedded userinfo, URLs containing embedded raw ASCII or Unicode whitespace, URLs containing percent-encoded ASCII spaces, or URLs containing raw or percent-encoded ASCII control bytes after trimming MUST be skipped instead of creating candidates with unsafe or non-clickable handoff provenance.
- **AND** optional Browse image URLs MUST be trimmed and preserved only when they are HTTP(S) URLs without embedded userinfo, oversized length, embedded raw ASCII or Unicode whitespace, percent-encoded ASCII spaces, or raw or percent-encoded ASCII control bytes; blank, relative, non-web, oversized, userinfo-bearing, encoded-space, raw-whitespace, or control-character image URLs MUST be dropped without rejecting an otherwise valid candidate.
