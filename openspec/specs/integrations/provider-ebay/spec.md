...
- **AND** normalized Browse price amounts MUST reject values containing raw or percent-encoded ASCII control bytes, Unicode control/format characters, raw non-ASCII whitespace, or percent-encoded ASCII/Unicode whitespace/control/format characters before trimming so unsafe upstream amount text cannot become a persisted scanner candidate price.
...
- **AND** shipping cost amount values containing raw or percent-encoded ASCII control bytes, Unicode control/format characters, raw non-ASCII whitespace, or percent-encoded ASCII/Unicode whitespace/control/format characters MUST be ignored before selecting the first valid same-currency shipping option.
