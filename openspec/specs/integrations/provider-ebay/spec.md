...
- **AND** structured upstream auth error payload detail fields MUST be whitespace-compacted and MUST omit fields containing raw or percent-encoded ASCII control bytes before Cabinet stores or returns provider diagnostics.
...
- **AND** structured non-auth Browse error payload detail fields MUST be whitespace-compacted and MUST omit fields containing raw or percent-encoded ASCII control bytes before Cabinet stores or returns provider diagnostics.
