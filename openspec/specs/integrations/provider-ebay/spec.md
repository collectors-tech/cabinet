...
- **AND** the adapter MUST trim and ignore keyword or exclusion criteria containing percent-encoded Unicode whitespace, control, or format characters before building the Browse request, and MUST reject a saved query that has no safe non-blank keyword criteria without calling eBay.
...
- **AND** Browse item summaries with listing id or title values containing percent-encoded Unicode whitespace, control, or format characters after trimming MUST be skipped instead of creating candidates with unsafe or visually ambiguous encoded text provenance.
...
- **AND** seller usernames containing percent-encoded Unicode whitespace, control, or format characters after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
