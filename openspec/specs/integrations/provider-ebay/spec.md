...
- **AND** configured Browse base URL overrides MUST be trimmed and accepted only when they are HTTP(S) URLs without embedded userinfo, query strings, fragments, embedded raw ASCII or Unicode whitespace, percent-encoded ASCII spaces, percent-encoded Unicode whitespace or format/control characters, or raw or percent-encoded ASCII control bytes; blank, relative, non-web, userinfo-bearing, query-bearing, fragment-bearing, encoded-space, encoded-unsafe-Unicode, raw-whitespace, or control-character override values MUST fall back to the default production eBay API base URL before authenticated requests are built.
...
- **AND** Browse item summaries with non-HTTP(S) item URLs, oversized item URLs, URLs containing embedded userinfo, URLs containing embedded raw ASCII or Unicode whitespace, URLs containing percent-encoded ASCII spaces, URLs containing percent-encoded Unicode whitespace or format/control characters, or URLs containing raw or percent-encoded ASCII control bytes after trimming MUST be skipped instead of creating candidates with unsafe or non-clickable handoff provenance.
- **AND** optional Browse image URLs MUST be trimmed and preserved only when they are HTTP(S) URLs without embedded userinfo, oversized length, embedded raw ASCII or Unicode whitespace, percent-encoded ASCII spaces, percent-encoded Unicode whitespace or format/control characters, or raw or percent-encoded ASCII control bytes; blank, relative, non-web, oversized, userinfo-bearing, encoded-space, encoded-unsafe-Unicode, raw-whitespace, or control-character image URLs MUST be dropped without rejecting an otherwise valid candidate.
