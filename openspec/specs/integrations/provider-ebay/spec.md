...
- **AND** when Browse item summaries include `shippingOptions.shippingCost.value`, the adapter MUST preserve the first parseable positive amount in the shared scanner candidate `shipping` field, ignoring blank, unparseable, zero, or negative shipping options before falling back to `0`.
