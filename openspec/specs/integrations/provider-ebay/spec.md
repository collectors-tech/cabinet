...
- **AND** malformed successful Browse response payloads, including valid first JSON objects followed by trailing JSON/data, MUST return an actionable `PROVIDER_SEARCH_FAILED` provider error instead of accepting partial upstream output or returning a generic decode failure so provider-run routes can preserve health guidance and retry behavior.
