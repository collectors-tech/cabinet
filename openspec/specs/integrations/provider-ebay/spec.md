...
- **AND** Browse item summaries with non-HTTP(S) item URLs, URLs containing embedded userinfo, or URLs containing percent-encoded ASCII control bytes after trimming MUST be skipped instead of creating candidates with unsafe or non-clickable handoff provenance.
- **AND** optional Browse image URLs MUST be trimmed and preserved only when they are HTTP(S) URLs without embedded userinfo or percent-encoded ASCII control bytes; blank, relative, non-web, userinfo-bearing, or encoded-control image URLs MUST be dropped without rejecting an otherwise valid candidate.
