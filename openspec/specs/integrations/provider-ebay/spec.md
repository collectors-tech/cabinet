...
- **AND** normalized Browse candidate metadata MUST preserve valid `itemCreationDate` and `itemEndDate` timestamps as normalized UTC scanner candidate fields, and MUST drop malformed or unsafe timestamp values without rejecting an otherwise valid candidate.
