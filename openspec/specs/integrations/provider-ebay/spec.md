...
- **AND** Browse item summaries with non-HTTP(S) item URLs, URLs containing embedded userinfo, URLs containing embedded raw whitespace, or URLs containing raw or percent-encoded ASCII control bytes after trimming MUST be skipped instead of creating candidates with unsafe or non-clickable handoff provenance.
