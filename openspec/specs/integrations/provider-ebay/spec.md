...
- **AND** normalized Browse candidate string metadata MUST trim surrounding whitespace from listing id, title, URL, image URL, seller username, and currency before the candidate is returned for persistence.
...
- **AND** deterministic request/profile resolution failures MUST be documented through a reusable client-error schema for invalid JSON, missing query set id, missing active profile, settings lookup failure, invalid query set id, and candidate reload failure.
...
- **AND** the contract MUST document rejected eBay credentials as `403` responses using the same reusable auth-error envelope so generated clients can distinguish forbidden credentials from missing credentials without losing the provider/auth diagnostic fields.
...
- **AND** the contract MUST document unsupported method requests with `405` and `error="method_not_allowed"` so generated clients do not infer that other methods are accepted for provider runs.
