...
- **AND** rejected eBay credentials with non-JSON or plain-text upstream bodies MUST preserve bounded body details alongside the local status message so operators can distinguish provider scope, gateway, and account failures from generic credential rejection.
...
- **AND** non-auth Browse failures with non-JSON or plain-text upstream bodies MUST preserve bounded body details alongside the local status message while retaining the local `PROVIDER_SEARCH_FAILED` classification.
