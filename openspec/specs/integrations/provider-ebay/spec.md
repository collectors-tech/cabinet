...
- **AND** normalized Browse candidate metadata MUST preserve valid `itemCreationDate` and `itemEndDate` timestamps as normalized UTC scanner candidate fields, and MUST drop malformed timestamp values or timestamp values containing raw or percent-encoded ASCII control bytes, raw Unicode control/format characters, or percent-encoded Unicode whitespace/control/format characters without rejecting an otherwise valid candidate.
