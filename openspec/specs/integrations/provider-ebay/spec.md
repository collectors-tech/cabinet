...
- **AND** Browse item summaries with non-HTTP(S) item URLs or URLs containing embedded userinfo after trimming MUST be skipped instead of creating candidates with unsafe or non-clickable handoff provenance.
- **AND** optional Browse image URLs MUST be trimmed and preserved only when they are HTTP(S) URLs without embedded userinfo; blank, relative, non-web, or userinfo-bearing image URLs MUST be dropped without rejecting an otherwise valid candidate.
