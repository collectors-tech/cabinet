...
- **AND** bearer tokens containing raw or percent-encoded ASCII control bytes, Unicode control/format characters, or percent-encoded Unicode whitespace/control/format characters after trimming MUST return a local `PROVIDER_AUTH_INVALID` error before Cabinet builds or sends an eBay Browse request.
