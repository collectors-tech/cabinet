...
- **AND** configured Browse base URL overrides MUST be trimmed and accepted only when they are origin-only HTTP(S) URLs without path prefixes, embedded userinfo, query strings, fragments, malformed percent escapes in any URL component before parsing, embedded raw ASCII or Unicode whitespace, raw Unicode control/format characters, percent-encoded ASCII spaces, percent-encoded Unicode whitespace or format/control characters, or raw or percent-encoded ASCII control bytes; blank, relative, non-web, path-bearing, userinfo-bearing, query-bearing, fragment-bearing, malformed-escape, encoded-space, encoded-unsafe-Unicode, raw-unsafe-Unicode, raw-whitespace, or control-character override values MUST fall back to the default production eBay API base URL before authenticated requests are built.
...
- **AND** malformed marketplace IDs containing unsupported characters, missing the `EBAY_` prefix, containing non-ASCII characters, or containing raw or percent-encoded ASCII/Unicode unsafe text before trimming MUST fall back to the default `EBAY_US` marketplace before the adapter sends headers or derives Browse price-currency filters.
...
- **AND** eBay-scoped saved searches MUST use the eBay Browse page-size maximum of `200` as the shared scanner effective `items_per_page` cap, so provider-run and scheduled-run execution do not clamp valid eBay Browse requests to another provider's lower default cap.
- **AND** the adapter MUST cap normalized candidates to the effective Browse `limit` when upstream returns more valid item summaries than requested, so oversized provider pages cannot persist more candidates than the saved-query page-size contract.
- **AND** scheduled scanner execution MUST invoke the eBay provider only for enabled scheduled query sets whose `provider_scope` includes `ebay`, skipping unrelated provider scopes without recording eBay run history, candidate output, provider health failures, or scheduled summary failures for those unrelated saved searches.
...
- **AND** the adapter MUST trim and ignore blank keyword or exclusion criteria and criteria containing raw or percent-encoded ASCII control bytes before building the Browse request, and MUST reject a saved query that has no safe non-blank keyword criteria without calling eBay.
- **AND** the adapter MUST trim and ignore keyword or exclusion criteria containing raw Unicode whitespace, Unicode control, or format characters before building the Browse request, and MUST reject a saved query that has no safe non-blank keyword criteria without calling eBay.
- **AND** the adapter MUST trim and ignore keyword or exclusion criteria containing percent-encoded ASCII or Unicode whitespace, control, or format characters before building the Browse request, and MUST reject a saved query that has no safe non-blank keyword criteria without calling eBay.
- **AND** the adapter MUST trim and ignore keyword or exclusion criteria containing nested percent-encoded ASCII or Unicode whitespace, control, or format characters before building the Browse request, and MUST reject a saved query that has no safe non-blank keyword criteria without calling eBay.
- **AND** the adapter MUST trim and ignore keyword or exclusion criteria longer than the bounded provider text-field limit before building the Browse request, and MUST reject a saved query that has no safe non-blank keyword criteria without calling eBay.
- **AND** provider run APIs MUST map saved queries with no safe non-blank eBay keyword criteria to `PROVIDER_QUERY_INVALID` with `next_action="edit_ebay_query_criteria"` so clients can route operators back to saved-search editing instead of provider credentials or generic scanner failure recovery.
...
- **AND** normalized Browse candidate metadata MUST preserve valid `itemCreationDate` and `itemEndDate` timestamps as normalized UTC scanner candidate fields, and MUST drop malformed timestamp values, timestamp values containing malformed percent escapes before parsing, implausibly future timestamp values, impossible pairs whose creation timestamp is after the end/update timestamp, or timestamp values containing raw or percent-encoded ASCII control bytes, raw Unicode control/format characters, or percent-encoded Unicode whitespace/control/format characters without rejecting an otherwise valid candidate.
- **AND** normalized Browse price amounts MUST prefer `price` and then fall back to `currentBidPrice` for auction-style item summaries, while rejecting values containing raw or percent-encoded ASCII control bytes, malformed percent-escape sequences, Unicode control/format characters, raw non-ASCII whitespace, or percent-encoded ASCII/Unicode whitespace/control/format characters before trimming so unsafe upstream amount text cannot become a persisted scanner candidate price.
- **AND** when the primary Browse `price` amount is unsafe, malformed, unparseable, or otherwise unusable, the adapter MUST continue evaluating `currentBidPrice` so auction-style item summaries with a safe bid amount can still become saved-search candidates while unsafe bid fallbacks remain skipped.
...
- **AND** saved-query max-price criteria MUST be applied to Browse filters and local candidate filtering only when the value is positive and finite, so non-finite values such as `NaN` or `Infinity` cannot produce invalid provider price filters.
...
- **AND** Browse item summaries with price currency values containing raw or percent-encoded ASCII control bytes, raw Unicode whitespace, Unicode control/format characters, or percent-encoded Unicode whitespace/control/format characters before trimming MUST be skipped instead of creating candidates with unsafe observed currency provenance.
...
- **AND** Browse item summaries with listing id or title values containing Unicode control or format characters after trimming MUST be skipped instead of creating candidates with unsafe or visually ambiguous text provenance.
- **AND** Browse item summaries with listing id or title values containing percent-encoded ASCII or Unicode whitespace, control, or format characters after trimming MUST be skipped instead of creating candidates with unsafe or visually ambiguous encoded text provenance.
- **AND** Browse item summaries with listing id or title values containing nested percent-encoded ASCII or Unicode whitespace, control, or format characters after trimming MUST be skipped instead of creating candidates with unsafe or visually ambiguous encoded text provenance.
- **AND** Browse item summaries with listing id or title values containing malformed percent-escape sequences after trimming MUST be skipped instead of creating candidates with ambiguous encoded text provenance.
- **AND** Browse item summaries with listing id or title values longer than the bounded provider text-field limit after trimming MUST be skipped instead of creating oversized scanner candidates.
- **AND** duplicate Browse item summaries with the same listing id after trimming and case-insensitive comparison MUST be emitted only once per provider result set, preserving the first valid candidate's original listing-id casing and allowing earlier invalid duplicates to fall through so a single eBay listing cannot create duplicate scanner candidates in one run or suppress the first valid summary.
- **AND** duplicate Browse item summaries with the same normalized item URL after lowercasing scheme/host, stripping default HTTP/HTTPS ports, stripping fragments, and case-insensitive URL comparison MUST be emitted only once per provider result set, preserving the first valid candidate and allowing earlier invalid duplicate-URL summaries to fall through so variant listing IDs cannot create duplicate scanner candidates for the same eBay listing URL.
- **AND** Browse item summaries with non-HTTP(S) item URLs, oversized item URLs, URLs containing embedded userinfo, malformed percent-encoded escapes detected before parsing, embedded raw ASCII or Unicode whitespace before trimming, raw Unicode control/format characters, URLs containing percent-encoded ASCII spaces, URLs containing percent-encoded Unicode whitespace or format/control characters, URLs containing nested or deeply nested percent-encoded ASCII or Unicode whitespace/control/format characters, or URLs containing raw or percent-encoded ASCII control bytes after trimming MUST be skipped instead of creating candidates with unsafe or non-clickable handoff provenance.
- **AND** optional Browse image URLs MUST be trimmed, lowercased for scheme/host, stripped of default HTTP/HTTPS ports and fragments, and preserved only when they are HTTP(S) URLs without embedded userinfo, malformed percent-encoded escapes detected before parsing, oversized length, embedded raw ASCII or Unicode whitespace before trimming, raw Unicode control/format characters, percent-encoded ASCII spaces, percent-encoded Unicode whitespace or format/control characters, nested percent-encoded ASCII or Unicode whitespace/control/format characters, or raw or percent-encoded ASCII control bytes; blank, relative, non-web, oversized, userinfo-bearing, malformed-escape, encoded-space, encoded-unsafe-Unicode, nested-encoded-unsafe, raw-unsafe-Unicode, raw-whitespace, or control-character image URLs MUST be dropped without rejecting an otherwise valid candidate.
- **AND** when the primary Browse image URL is blank or unsafe, the adapter MUST preserve the first safe URL from `thumbnailImages` before `additionalImages` so candidates keep available visual provenance without accepting unsafe image URLs.
...
- **AND** seller usernames containing raw or percent-encoded ASCII control bytes after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** seller usernames containing Unicode control or format characters after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** seller usernames containing percent-encoded ASCII or Unicode whitespace, control, or format characters after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** seller usernames containing nested percent-encoded ASCII or Unicode whitespace, control, or format characters after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** seller usernames containing malformed percent-escape sequences after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** seller usernames containing embedded raw ASCII or Unicode whitespace after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** seller usernames longer than the bounded provider text-field limit after trimming MUST fall back to `seller="ebay"` without rejecting an otherwise valid candidate.
- **AND** when Browse item summaries include `shippingOptions.shippingCost.value`, the adapter MUST preserve the first parseable positive amount whose non-blank three-letter ASCII `shippingCost.currency` matches the normalized candidate price currency in the shared scanner candidate `shipping` field, ignoring mismatched-currency, malformed-currency, blank-currency, raw Unicode-whitespace or other raw/percent-encoded unsafe-currency text, blank-value, unparseable, zero, or negative shipping options before falling back to `0`.
- **AND** shipping cost amount values containing raw or percent-encoded ASCII control bytes, malformed percent-escape sequences, Unicode control/format characters, raw non-ASCII whitespace, or percent-encoded ASCII/Unicode whitespace/control/format characters MUST be ignored before selecting the first valid same-currency shipping option.
...
- **AND** shipping amounts MUST reject exponent notation, leading plus signs, comma characters after the decimal separator, partial decimal values missing digits before or after the decimal separator, and values with more than two fractional currency digits before selecting the first valid same-currency shipping option.
...
- **AND** malformed successful Browse response payloads, including missing or null `itemSummaries` arrays and valid first JSON objects followed by trailing JSON/data, MUST return an actionable `PROVIDER_SEARCH_FAILED` provider error instead of accepting partial upstream output or returning a generic decode failure so provider-run routes can preserve health guidance and retry behavior.
- **AND** successful Browse response payloads larger than the bounded provider response-body limit MUST return an actionable `PROVIDER_SEARCH_FAILED` provider error instead of decoding unbounded upstream output so provider-run routes can preserve health guidance and avoid persisting partial provider data.
- **AND** when a saved query includes max price, region, or condition criteria, the adapter MUST translate those criteria into documented Browse field filters before calling eBay: `price` with matching `priceCurrency`, `itemLocationCountry`, and broad `conditions` values where Cabinet can map the saved condition safely, including the user-facing `UK` region alias as eBay `GB` location country and common operator aliases such as `new with tags` and `pre-owned`.
- **AND** marketplace-derived Browse price currency filters MUST cover supported eBay marketplace IDs for Australia, Canada, United Kingdom, European Union euro marketplaces, Switzerland, Hong Kong, Malaysia, Philippines, Poland, Singapore, and United States so max-price filtering does not silently fall back to USD for known non-US marketplaces.
...
- **AND** saved-query region and condition criteria containing raw ASCII control bytes, raw Unicode whitespace, Unicode control or format characters, percent-encoded ASCII/Unicode whitespace/control/format characters, or nested percent-encoded unsafe text MUST be omitted before trimming or mapping so unsafe input cannot become a valid Browse filter.
...
- **AND** scanner candidate persistence MUST store normalized eBay source listing timestamps in `scanner_candidates.listing_created_at` and `scanner_candidates.listing_updated_at` for both newly inserted and refreshed candidates.
...
- **AND** shared scanner candidate read APIs MUST expose persisted normalized source listing timestamps as `listing_created_at` and `listing_updated_at` in eBay provider run and candidate-list responses.
...
- **AND** bearer tokens containing raw or percent-encoded ASCII control bytes, raw Unicode whitespace, Unicode control/format characters, or percent-encoded Unicode whitespace/control/format characters after trimming MUST return a local `PROVIDER_AUTH_INVALID` error before Cabinet builds or sends an eBay Browse request.
- **AND** bearer tokens longer than the bounded credential length limit MUST return a local `PROVIDER_AUTH_INVALID` error before Cabinet builds or sends an eBay Browse request, so oversized credential payloads cannot be forwarded to eBay or provider logs.
- **AND** structured upstream auth error payload detail fields MUST be whitespace-compacted, length-bounded, and MUST omit fields containing raw or percent-encoded ASCII control bytes before Cabinet stores or returns provider diagnostics.
- **AND** structured upstream auth error payload detail fields MUST omit fields containing Unicode control or format characters before Cabinet stores or returns provider diagnostics.
- **AND** structured upstream auth error payload detail fields MUST omit fields containing percent-encoded Unicode whitespace, control, or format characters before Cabinet stores or returns provider diagnostics.
- **AND** structured upstream auth error payload detail fields MUST omit fields containing nested percent-encoded ASCII or Unicode whitespace, control, or format characters before Cabinet stores or returns provider diagnostics.
- **AND** rejected eBay credentials with non-JSON or plain-text upstream bodies MUST cap the upstream body read before diagnostics are built and preserve bounded, whitespace-compacted body details alongside the local status message so operators can distinguish provider scope, gateway, and account failures from generic credential rejection.
...
- **AND** structured non-auth Browse error payload detail fields MUST be whitespace-compacted, length-bounded, and MUST omit fields containing raw or percent-encoded ASCII control bytes before Cabinet stores or returns provider diagnostics.
- **AND** structured non-auth Browse error payload detail fields MUST omit fields containing Unicode control or format characters before Cabinet stores or returns provider diagnostics.
- **AND** structured non-auth Browse error payload detail fields MUST omit fields containing percent-encoded Unicode whitespace, control, or format characters before Cabinet stores or returns provider diagnostics.
- **AND** structured non-auth Browse error payload detail fields MUST omit fields containing nested percent-encoded ASCII or Unicode whitespace, control, or format characters before Cabinet stores or returns provider diagnostics.
- **AND** non-auth Browse failures with non-JSON or plain-text upstream bodies MUST cap the upstream body read before diagnostics are built and preserve bounded, whitespace-compacted body details alongside the local status message while retaining the local `PROVIDER_SEARCH_FAILED` classification.
...
- **AND** when eBay Browse returns a positive integer or future HTTP-date `Retry-After` header on a provider search failure, provider run APIs MUST preserve it as `retry_after_seconds` so clients can show deterministic retry timing instead of generic recovery text only, while capping excessive upstream retry timing at 24 hours before exposing it to clients.
...
- **AND** the provider-specific run route MUST reject malformed JSON, including request bodies with extra trailing JSON/data after the first object, missing active-profile state, and profile settings lookup failures with structured client-error envelopes including `provider="ebay"`, the parsed or empty `query_set_id`, and deterministic `next_action` recovery values before reading saved searches or calling Browse.
...
- **AND** the contract MUST document saved-query criteria failures with `error="failed_to_run_ebay_provider"`, `PROVIDER_QUERY_INVALID`, `query_set_id`, and `next_action="edit_ebay_query_criteria"`.
...
- **AND** the setup UI MUST always render the eBay Browse base URL routing state, distinguishing the default production Browse API base URL from an explicitly configured override without exposing the override value.
