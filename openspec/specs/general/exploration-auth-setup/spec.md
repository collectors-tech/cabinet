## Purpose
Define deterministic exploratory auth setup guidance for Cabinet so route audits and UI review do not stall on avoidable local-vs-ZITADEL ambiguity.
...
- **GIVEN** a developer or reviewer is starting exploratory work not explicitly scoped to ZITADEL behavior
...
### Requirement EXPLORATION-AUTH-SETUP-002: Exploratory auth guidance SHALL distinguish ZITADEL-specific prerequisites and blockers
Cabinet documentation SHALL distinguish ZITADEL-only auth prerequisites from the default local exploratory path so provider configuration gaps are not confused with general product failures.

#### Scenario: Resolve ZITADEL exploratory prerequisites
- **GIVEN** exploratory work explicitly targets ZITADEL flows or permissions behavior
- **WHEN** the reviewer follows the exploratory auth setup guide
- **THEN** the guide MUST enumerate ZITADEL auth-mode, issuer, client-id and audience prerequisites
- **AND** the guide MUST identify common ZITADEL/domain/origin blockers with actionable next steps
- **AND** the guide MUST include a concrete repo-level ZITADEL startup example and verification target
...
### Requirement EXPLORATION-AUTH-SETUP-006: Exploratory auth guidance SHALL document the ZITADEL runtime contract
Cabinet documentation SHALL provide deterministic ZITADEL exploration startup guidance so reviewers can start a ZITADEL-oriented runtime with explicit env expectations and verification targets.

#### Scenario: Start ZITADEL exploration runtime from repo guidance
- **GIVEN** the reviewer needs ZITADEL-specific exploratory auth coverage
- **WHEN** they start Cabinet with `CABINET_AUTH_IDENTITY_MODE=zitadel` and required `CABINET_ZITADEL_*` values
- **THEN** the guide MUST identify the expected runtime URL and verification endpoints
- **AND** the guide MUST identify provider-options evidence for `identity_mode=zitadel` and configured ZITADEL state
- **AND** the active runtime scripts MUST NOT provide a retired provider-specific exploration launcher
