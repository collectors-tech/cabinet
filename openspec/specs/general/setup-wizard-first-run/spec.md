...
| UC-SW-24 | ZITADEL readiness | ZITADEL mode exposes no editable third-party key field and allows progression without manual key entry | implemented: `ui.web/cypress/e2e/general/setup-wizard-first-run/spec.cy.ts` `UC-SW-24 setup-wizard-zitadel-readiness allows next without Clerk key entry`; `SETUP-WIZ-015 + #1968 keeps retired Clerk key out of setup payload` |
