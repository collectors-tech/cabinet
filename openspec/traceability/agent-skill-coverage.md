...
| Chats / Agent itself | `cabinet.chat.action_timeline.view`, `cabinet.navigate.open_surface`, `cabinet.agent.explain_available_work` | "What can you do here?", "Show the action timeline", "Open inventory" | read-only, preview-only | Profile; thread; route/source context | None | chat-action-timeline planned, navigate-open-surface built-in seed | None | partial | #1703, #1714, #1716 | `openspec/traceability/agent-acceptance-suite.md`; `internal/app/chat_api_test.go` | Universal context envelope remains planned in #1714 |
...
These seed entries are not enough to close #1701 or any child skill issue by themselves. Child issues #1707, #1708, #1709, #1710, #1711, and #1715 own execution behavior, preview/apply tests, UI/channel coverage where applicable, and closure evidence for each surface group. The #1710 slice makes Market Watch and Purchases skills visible with safety/status metadata, non-mutating provider/order/item blockers, profile-scoped Market Watch saved-watch create/update persistence through `scanner_query_sets`, saved-watch search reload evidence, stubbed provider-health run evidence, provenance-preserving Market Watch handoff persistence into Wishlist/Purchases/Inventory destination state, purchase create-order and add-line apply persistence into commerce lifecycle/expected-arrival records, read-only purchase-order search reload evidence, purchase receive-order bulk delivery persistence, purchase receive-line delivery persistence, and reconciliation persistence; live provider dispatch and UI/channel dispatch remain separate follow-up work. The #1715 slice makes Inbox and Users/admin skills visible with safety/status metadata, non-mutating preview blockers, direct confirmed apply handlers for Inbox status changes plus Users/admin mutations with protected owner/admin enforcement, cancel/no-confirm safeguards, unknown-skill rejection, and preview/apply source surface/channel/thread/message propagation; broader UI shell/channel dispatcher UX remains separate.
