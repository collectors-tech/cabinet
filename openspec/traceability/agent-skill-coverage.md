...
| Inbox review | Notification/review item, source thread, preview handoff | partial | #1715 | `internal/agentskills/registry_test.go` and `internal/app/agent_skills_api_test.go` cover Inbox skill registry metadata; preview/apply execution remains planned |
...
| Inbox | `cabinet.inbox.search_notifications`, `cabinet.inbox.summarise_unhandled`, `cabinet.inbox.open_notification`, `cabinet.inbox.mark_handled`, `cabinet.inbox.archive_or_hide`, `cabinet.inbox.route_to_surface` | "Summarise unhandled Inbox items", "Mark this handled" | read-only, preview-only, confirm-required | Profile; notification selection for mutation | None | navigate-open-surface for route/open skills; notification mutation handlers planned | None | partial | #1715 | `internal/agentskills/registry_test.go` (`TestInboxAndUsersAdminSkillsExposeSafetyAndExecutionBoundaries`, `TestSkillPreviewBlocksUnboundInboxAndUsersMutations`); `internal/app/agent_skills_api_test.go` registry API assertions | State-changing skill previews now block missing targets and unbound apply; confirmed apply handlers still planned |
| Users / workspace admin | `cabinet.users.search`, `cabinet.users.invite_user`, `cabinet.users.resend_invitation`, `cabinet.users.update_role`, `cabinet.users.activate_or_deactivate`, `cabinet.users.remove_user` | "Invite this user", "Deactivate this account" | read-only, confirm-required, destructive | Profile/workspace admin context; exact target user | Backend support and role permissions | users-admin mutation handlers planned | None | partial | #1715 | `internal/agentskills/registry_test.go` (`TestInboxAndUsersAdminSkillsExposeSafetyAndExecutionBoundaries`, `TestSkillPreviewBlocksUnboundInboxAndUsersMutations`); `internal/app/agent_skills_api_test.go` (`TestAgentSkillRegistryAPIExposesGovernedSkillMetadata`, `TestAgentSkillPreviewAPIBlocksUnsafeAdminMutation`) | Preview blocks missing target and protected owner/local admin removal/downgrade; confirmed apply handlers remain planned |
...
These seed entries are not enough to close #1701 or any child skill issue by themselves. Child issues #1707, #1708, #1709, #1710, #1711, and #1715 own execution behavior, preview/apply tests, UI/channel coverage where applicable, and closure evidence for each surface group. The #1715 seed and preview slice makes Inbox and Users/admin skills visible with safety/status metadata and non-mutating preview blockers, but deliberately leaves Inbox state changes and Users admin mutations non-executable until confirmed apply handlers are proven.
