...
func TestExpiredAndDowngradedEntitlementsRetainReadAndExportAccess(t *testing.T) {
	t.Parallel()
	...
	if _, err := a.db.ExecContext(context.Background(), `INSERT INTO app_state(key, value, updated_at) VALUES('cloud.plan', 'pro', CURRENT_TIMESTAMP)`); err != nil {
		t.Fatalf("set pro cloud plan: %v", err)
	}
	itemResp := doRequest(t, a, http.MethodPost, "/api/items", strings.NewReader(`{"brand":"AFX","category":"Slot","part_number":"RET-001","title":"Retained entitlement item"}`), map[string]string{"Content-Type": "application/json"})
	if itemResp.Code != http.StatusCreated {
		t.Fatalf("create item status=%d body=%s", itemResp.Code, itemResp.Body.String())
	}

	if _, err := a.db.ExecContext(context.Background(), `UPDATE app_state SET value = 'free', updated_at = CURRENT_TIMESTAMP WHERE key = 'cloud.plan'`); err != nil {
		t.Fatalf("downgrade cloud plan: %v", err)
	}
	payload := licensing.Payload{
		ProfileID: p.ID,
		Tier:      "pro",
	...
	statusResp := doRequest(t, a, http.MethodGet, "/api/license/status?profile_id="+p.ID, nil, nil)
	if statusResp.Code != http.StatusOK || !strings.Contains(statusResp.Body.String(), `"state":"expired"`) || !strings.Contains(statusResp.Body.String(), `"tier":"free"`) {
		t.Fatalf("expected expired license to resolve as free read/export state, status=%d body=%s", statusResp.Code, statusResp.Body.String())
	}

	readResp := doRequest(t, a, http.MethodGet, "/api/items", nil, nil)
	if readResp.Code != http.StatusOK || !strings.Contains(readResp.Body.String(), "Retained entitlement item") {
		t.Fatalf("downgraded read access status=%d body=%s", readResp.Code, readResp.Body.String())
	}
	jsonExport := doRequest(t, a, http.MethodGet, "/api/data/export/json", nil, nil)
	if jsonExport.Code != http.StatusOK || !strings.Contains(jsonExport.Body.String(), "Retained entitlement item") {
		t.Fatalf("downgraded json export status=%d body=%s", jsonExport.Code, jsonExport.Body.String())
	}
	csvExport := doRequest(t, a, http.MethodGet, "/api/data/export/csv/items", nil, nil)
	if csvExport.Code != http.StatusOK || !strings.Contains(csvExport.Body.String(), "Retained entitlement item") {
		t.Fatalf("downgraded csv export status=%d body=%s", csvExport.Code, csvExport.Body.String())
	}
}
