describe('chats/assistant-workspace', () => {
  ...
  it('ASSISTANT-WORKSPACE-008/#1509 starts a show-mode guided item-update walkthrough without mutation', () => {
    ...
    cy.get('[data-testid="shell-assistant-command-event"]')
      ...
      .last()
      .should('have.attr', 'data-command-status', 'success')
      .and('contain', 'Walkthrough resumed')
    ...
    cy.get('[data-testid="shell-assistant-guided-back"]').click()
    cy.get('[data-testid="shell-assistant-command-event"]')
      .filter('[data-command-type="walkthrough.step_back"]')
      .last()
      .should('have.attr', 'data-command-status', 'success')
      .and('contain', 'Returned to previous checkpoint')
    cy.get('[data-testid="shell-assistant-guided-skip"]').click()
    cy.get('[data-testid="shell-assistant-command-event"]')
      .filter('[data-command-type="walkthrough.skip"]')
      .last()
      .should('have.attr', 'data-command-status', 'success')
      .and('contain', 'Skipped to next safe checkpoint')
