(() => {
  if (globalThis.__cabinetFrontlineCaptureV1) return
  globalThis.__cabinetFrontlineCaptureV1 = true

  const moduleID = 'frontlinehobbies-search-capture'
  const productCardSelector = 'li.product, article.product, .product-item, [data-product-id], [data-product-sku], [itemtype="https://schema.org/Product"], [itemtype="http://schema.org/Product"]'
  const challengeSelectors = ['form#challenge-form', '.cf-challenge', '[data-cabinet-provider-state="challenge"]']
  const loggedOutSelectors = ['form.woocommerce-form-login input[type=password]', 'form[action*="login"] input[type=password]']
  ...
  const safeURL = (raw, base, image = false) => {
    let parsed
    try { parsed = new URL(String(raw ?? ''), base) } catch { return '' }
    const productHosts = new Set(['frontlinehobbies.com.au', 'www.frontlinehobbies.com.au'])
    const imageHosts = new Set([...productHosts, 'cdn.frontlinehobbies.com.au'])
    if (parsed.protocol !== 'https:' || parsed.username || parsed.password || !(image ? imageHosts : productHosts).has(parsed.hostname)) return ''
    if (!image) parsed.hostname = 'www.frontlinehobbies.com.au'
    parsed.search = ''
    parsed.hash = ''
    return parsed.href
  }
  ...
  const productItem = (card, pageURL) => {
    ...
    const link = firstNode(card, [
      'a.woocommerce-LoopProduct-link', 'a.product-item-link', 'a[href*="frontlinehobbies.com.au"]', 'a[href]',
    ])
    const titleNode = firstNode(card, [
      '.woocommerce-loop-product__title', '.product-item-name', '[itemprop="name"]', 'h2', 'h3',
    ])
    ...
    const imageNode = firstNode(card, [
      'img.attachment-woocommerce_thumbnail', 'img.product-image-photo', 'img[itemprop="image"]', 'img',
    ])
    ...
    const imageURL = safeURL(imageNode?.currentSrc || imageNode?.src || attribute(imageNode, 'data-src') || attribute(imageNode, 'src'), pageURL, true)
    ...
    const listingID = bounded(
      attribute(card, 'data-product-id') || attribute(card, 'data-product-sku') ||
      attribute(firstNode(card, ['[data-product-id]', '[data-product-sku]', '[itemprop="sku"]']), 'data-product-id') ||
      attribute(firstNode(card, ['[data-product-sku]', '[itemprop="sku"]']), 'data-product-sku') ||
  ...
  const pageNumber = (pageURL, documentRef) => {
    const fromURL = Number(pageURL.searchParams.get('paged') || pageURL.searchParams.get('product-page') || pageURL.searchParams.get('page'))
  ...
  const findState = (documentRef) => {
    if (challengeSelectors.some((selector) => documentRef.querySelector(selector))) return 'challenge'
  ...
  const capture = (documentRef, pageURL, explicitQuery = '', paginationOverride) => {
    ...
    if (state === 'challenge') return { state, error: 'frontline_challenge_action_required' }
    if (state === 'logged_out') return { state, error: 'frontline_login_required' }
    if (state !== 'ready') return { state: 'unsupported', error: 'frontline_selector_drift' }
    ...
    if (items.length === 0) return { state: 'unsupported', error: 'frontline_selector_drift' }
    ...
    const query = bounded(explicitQuery || pageURL.searchParams.get('s') || pageURL.searchParams.get('q') || documentRef.title || 'Frontline search', 512)
  ...
  if (typeof globalThis.__cabinetFrontlineCaptureTestHooks === 'function') {
    globalThis.__cabinetFrontlineCaptureTestHooks({ capture, parsePrice, productItem, safeURL })
  }
