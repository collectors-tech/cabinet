...
The popup shows setup, sync, pause/resume and review actions only when the Cabinet module contract proves they are available. Cabinet currently advertises `sync_available: false`: #2032 must supply durable item/image persistence before the host can enable **Sync now**. When enabled later, the core loads the separately packaged module capture script, accepts only declared scalar fields, strips query/fragment data from source URLs, rejects raw page/session fields, and queues the passive envelope for Cabinet review.
