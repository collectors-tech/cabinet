...
- Build or nominate `dist/cabinet/cabinet-0.1.0-beta.6-windows-amd64-portable.zip` from a clean commit.
...
- Verify `/api/runtime` reports `app_version=0.1.0-beta.6`, the runtime port, process id, data directory, build date, and a full lowercase `build_revision` that exactly equals the Cabinet manifest `source_commit`.
