...
| Attachment success/failure | AGENT-UNIVERSAL-CHANNELS-003 | planned | `internal/app/chat_api_test.go` (`TestAgentChatAttachmentsRemainScopedAndExplicit`); `ui.web/cypress/e2e/chats/agent-attachments/spec.cy.ts` (`AGENT-ATTACHMENTS-001 handles main and side-panel attachments consistently`) | Supported attachment is scoped to profile/thread/message with provenance; unsupported attachment returns deterministic validation guidance without unrelated records. |
