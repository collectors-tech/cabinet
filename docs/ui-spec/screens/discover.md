...
3. User reviews stock status/count to prioritize buy timing.
4. User triages each candidate via ignore/wishlist/track/create.

## UI Sections
1. Filter bar
2. Candidate list
3. Candidate row actions
4. Stock state chip (`In Stock`, `Low Stock`, `Out of Stock`, `Unknown`)
...
- [ ] Candidate rows render stock status when available.
- [ ] `Out of Stock` and `Low Stock` states are visually distinct.
...
- `DISC-005` stock chip rendering for in/out/low/unknown states.
