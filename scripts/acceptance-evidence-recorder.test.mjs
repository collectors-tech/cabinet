...
const candidateFixture = async ({ sourceCommit = commit, suffix = 'one' } = {}) => {
  ...
  const candidateVersion = `0.1.0-beta.1.g${sourceCommit.slice(0, 12)}`
  const cabinetArtifact = await writeArtifact(directory, 'cabinet-0.1.0-beta.1-windows-amd64-portable.zip', `cabinet-${suffix}`)
  ...
  const cabinet = {
    ...
    release_notes_filename: 'cabinet-0.1.0-beta.1-release-notes.md',
  }
  const companion = {
    schema_version: 1,
