...
const candidateFixture = async ({ sourceCommit = commit, suffix = 'one' } = {}) => {
  ...
  const candidateVersion = `0.1.0-beta.9.g${sourceCommit.slice(0, 12)}`
  const cabinetArtifact = await writeArtifact(directory, 'cabinet-0.1.0-beta.9-windows-amd64-portable.zip', `cabinet-${suffix}`)
  ...
  const cabinet = {
    ...
    release_notes_filename: 'cabinet-0.1.0-beta.9-release-notes.md',
  }
  const companion = {
    schema_version: 1,
...
const environment = {
  ...
  runtime: { app_version: '0.1.0-beta.9', build_revision: commit, build_date: '2026-08-11T00:00:00Z', port: 17900, pid: 4242 },
}
...
test('candidate identity, three manifests, and independently verified package checksums are mandatory', async () => {
  ...
  assert.equal(state.candidate.cabinet.package.filename, 'cabinet-0.1.0-beta.9-windows-amd64-portable.zip')
