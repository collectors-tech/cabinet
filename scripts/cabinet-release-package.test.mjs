...
const cabinetFixture = async () => {
  ...
  const filename = 'cabinet-0.1.0-beta.8-windows-amd64-portable.zip'
  const checksumFilename = `${filename}.sha256`
  const notesFilename = 'cabinet-0.1.0-beta.8-release-notes.md'
  ...
  await writeFile(join(directory, notesFilename), `# Cabinet 0.1.0-beta.8\n\nCommit: ${sourceCommit}\n\nWindows portable package; not an installer.\n\n${disclosureNotes}`)
  const manifest = {
    schema_version: 1,
    product: 'Cabinet',
...
test('creates one source-bound non-publishing Cabinet and companion bundle', async () => {
  ...
  await writeFile(companionPath, `${JSON.stringify({
    ...
    channel: 'private-beta',
    version_name: '0.1.0-beta.8.gcccccccccccc',
    source_commit: sourceCommit,
    publication_state: 'private_candidate_not_published',
