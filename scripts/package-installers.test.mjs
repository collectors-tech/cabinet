...
describe("Cabinet beta packaging contract", () => {
  it("defines one canonical beta version source", () => {
    ...
    assert.equal(payload.version, "0.1.0-beta.7");
  ...
  it("keeps the non-publishing artifact validation checklist bound to the beta release gate", () => {
    ...
    assert.match(checklist, /cabinet-0\.1\.0-beta\.7-windows-amd64-portable\.zip/);
    assert.match(checklist, /cabinet-0\.1\.0-beta\.7-windows-amd64-portable\.zip\.sha256/);
    assert.match(checklist, /cabinet-0\.1\.0-beta\.7-release-notes\.md/);
    ...
    assert.match(checklist, /app_version=0\.1\.0-beta\.7/);
